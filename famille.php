			...
			<![endif]-->
				<?php include('pages/header.php'); ?>
							...
							<?php
								...
								$databannier = mysql_fetch_array($res);
			...
			<figure class="m_bottom_10">
					<div class="flexslider">
						...
						<main style="" class="rightCol col-lg-2 col-md-8 col-sm-8 m_bottom_30 m_xs_bottom_10">
							<img style="" src="img/bannier/<?php echo ($databannier['bannier']); ?>" />		
						</main>
...
<p valign="justify" class="fw_light">
		...
		$dir = 'img/categories/'.suppr_accents($categorie).'/familles/'.utf8_encode(suppr_accents(ucfirst($famille))).'/';
 
		while($data_fa = mysql_fetch_array($res_fa))
		{
			
			$linkimages = $dir.utf8_encode($data_fa['libelle']).'/';
			?>
								
								<div class="col-lg-<?php echo $nombre ?> col-md-<?php echo $nombre ?> col-sm-<?php echo $nombre ?> col-xs-6">
									<div class="frame_container relative">
										<figure class="relative">
											<a href="product.php?id=<?php echo $data_fa['id']; ?>" class="d_block m_bottom_15">
												<img src="<?php echo ($linkimages.$data_fa['image']); ?>" alt=""></a>
											<figcaption>
												<a href="product.php?id=<?php echo $data_fa['id']; ?>" class="second_font sc_hover"><?php echo ((utf8_encode($data_fa['libelle']))); ?> </a><br />
		...
		if($nb_c == 8)
				$nombre = 4;
		$sql = "select * from produit where id_famile = $id_famile";
		$res_fa = mysql_query($sql);
