...
$sql_categ = "select * from categorie where id = '$id_categ'";
$res_categ = @mysql_query($sql_categ);
$data_categ = @mysql_fetch_array($res_categ);
?>
	<head>
		<title><?php echo $data_categ['categorie']; ?></title>
		<meta charset="utf-8">
...
// $(function() { 
 // $('html, body').animate({
    // scrollTop: $('#scroller').offset().top}, 1000);
// }); 
</script>
		...
		<!--layout-->
			...
			<![endif]-->
			 ...
			 <!--breadcrumbs-->
			<div id="scroller" class="breadcrumbs bg_grey_light_2 fs_medium fw_light">
				<div class="container">
					<a href="index.php" class="sc_hover">Acceuil</a> <span class="color_dark">/ Produits / </span> 	<?php echo ($data_categ['categorie']); ?>			</div>
			</div>
			<!--main content-->
			<div class="page_section_offset">
				<div class="container">
					...
					<main class="col-lg-7 col-md-8 col-sm-8 m_xs_bottom_10">
							
			<figure class="m_bottom_15 ">
						
					
					<h4 class="fw_light second_font color_dark tt_uppercase m_bottom_5" style="color:#ee7f01 !important;"> <strong> <?php echo ($data_categ['categorie']); ?></strong></h4>
									<hr class="fw_light second_font color_dark tt_uppercase m_bottom_10" />
							...
							<div class="flexslider">
								<ul class="slides">
									<?php 
										...
										$dir = 'img/categories/'.skip_accents($data_categ['categorie']).'/animation/*.{jpg,jpeg,gif,png}';
										$files = glob($dir,GLOB_BRACE);
 										foreach($files as $cle =>$valeur){
											
											echo'<li><img src="'.$valeur.'" ></li>';
...
<p class="fw_light">
			<?php echo ($data_categ['description']); ?>
							...
							<div class="row t_xs_align_c">
									...
									<?php 
								/*	
		$select_count = "select count(*) as nb_famille from famille where id_categ = $id_categ";
		...
		if($nb_c == 8)
				$nombre = 3;
		$sql = "select * from famille where id_categ = '$id_categ'";
		$res_fa = mysql_query($sql);
		
		while($data_fa = mysql_fetch_array($res_fa))
		{
			 $categorie = skip_accents(getNameCateg ($data_fa['id_categ']));
			 $famille = skip_accents($data_fa['famille']);
			?>
			...
			<div class="col-lg-<?php echo $nombre ?> col-md-<?php echo $nombre ?> col-sm-<?php echo $nombre ?> col-xs-<?php echo $nombre ?>">
				<div class="frame_container relative">
					<figure class="relative">
						<a href="famille.php?id=<?php echo $data_fa['id']; ?>&id_categ=<?php echo $id_categ; ?>" class="d_block m_bottom_15">
							<img src="img/categories/<?php echo $categorie; ?>/familles/<?php echo $famille; ?>/<?php echo $data_fa['image']; ?>" alt="">
						</a>
						<figcaption>
							<a href="famille.php?id=<?php echo $data_fa['id']; ?>&id_categ=<?php echo $id_categ; ?>" class="second_font sc_hover">
								<?php echo ($data_fa['famille']); ?>
							</a>
						</figcaption>
					</figure>
				</div>
		...
		<?php } */ ?> 
				...
				<main style="" class="rightCol col-lg-2 col-md-8 col-sm-8 m_bottom_30 m_xs_bottom_10">
					<img style="padding-top:40px;" src="img/bannier/<?php echo ($data_categ['bannier']); ?>" />		
				</main>
		...
		<div class="container">
			...
			<main class="col-lg-9 col-md-8 col-sm-8 m_bottom_30 m_xs_bottom_10">
						...
						<div class="row t_xs_align_c">
									...
									<?php 
		$select_count = "select count(*) as nb_famille from famille where id_categ = ".$_GET['id_categ'];
		$res_count = mysql_query($select_count);
		$select_count = mysql_fetch_array($res_count);
		$nb_c = ($select_count['nb_famille']);
		...
		if($nb_c == 8)
				$nombre = 3;
		$sql = "select * from famille where id_categ = ".$_GET['id_categ'];
		$res_fa = mysql_query($sql);
		...
		while($data_fa = mysql_fetch_array($res_fa))
		{
			...
			<div class="m_bottom_15 col-lg-<?php echo $nombre ?> col-md-<?php echo $nombre ?> col-sm-<?php echo $nombre ?> col-xs-6">
				<div class="frame_container relative">
					<figure class="relative">
						<a href="<?php echo ($data_fa['famille']); ?>-<?php echo $data_fa['id']; ?>-<?php echo $id_categ; ?>.html" class="d_block m_bottom_15">
							<img src="img/categories/<?php echo $categorie; ?>/familles/<?php echo $famille; ?>/<?php echo $data_fa['image']; ?>" alt="">
						</a>
						<figcaption>
							<a href="<?php echo ($data_fa['famille']); ?>-<?php echo $data_fa['id']; ?>-<?php echo $id_categ; ?>.html" class="second_font sc_hover">
								<?php echo ($data_fa['famille']); ?>
							</a>
						</figcaption>
					</figure>
